...
export default function BoutiquesPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase.from("shops").select("id, nom, ville, adresse, description, lien").order("nom");
    setShops(data ?? []);
  }
  ...
  useEffect(() => {
    ...
    async function fetchShops() {
      const { data } = await supabase
        .from("shops")
        .select("id, nom, ville, adresse, description, lien")
        .order("nom");
    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-8">
      ...
      {isModerator && (
        <form onSubmit={handleSubmit} className="mb-6 flex flex-col gap-3 rounded-lg border border-foreground/10 p-4">
          <h2 className="text-sm font-medium text-foreground/60">{t.shops.addShop}</h2>
          <input
            type="text"
            required
      ...
      <div className="flex flex-col gap-3">
        {shops !== null && filtered.length === 0 && (
          <p className="text-sm text-foreground/60">{t.shops.empty}</p>
        )}
        {filtered.map((shop) => (
          <div key={shop.id} className="rounded-lg border border-foreground/10 p-4">
            <h3 className="font-medium">{shop.nom}</h3>
            {shop.adresse && <p className="mt-1 text-sm text-foreground/60">{shop.adresse}</p>}
